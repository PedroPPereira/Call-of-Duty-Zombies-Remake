...
public class HandgunScriptLPFP : MonoBehaviour {
	...
	private void Update () {
        ...
        /*if (Input.GetKeyDown (KeyCode.Alpha1)) PauseMenu handles this
		{
        ...
        CharacterManager.currentAmmo = currentAmmo;
		...
		if (Input.GetKeyDown (KeyCode.V) && !isInspecting && isAiming) 
		{
		...
		//Shooting 
		if (Input.GetMouseButtonDown (0) && !outOfAmmo && !isReloading && !isInspecting && !isRunning && !PauseMenu.GameIsPaused) 
		{
			anim.Play ("Fire", 0, 0f);
		...
		if (Input.GetKeyDown (KeyCode.Q) && !hasBeenHolstered) 
		{
			holstered = true;
		...
		else if (Input.GetKeyDown (KeyCode.Q) && hasBeenHolstered) 
		{
			holstered = false;
