...
public class AIZombieStateMachine : AIStateMachine
{
	...
	{
        _health = 10 * GameManagement.round;
	...
	public override void TakeDamage( Vector3 position, Vector3 force, int damage, Rigidbody bodyPart, CharacterManager characterManager, int hitDirection=0 )
	{
        ...
        if (_boneControlType==AIBoneControlType.Ragdoll)
		{
			if (bodyPart!=null)
			{
                ...
                {
                    GameManagement.zombiesRemaining -= 1;
                    Destroy(gameObject);
                }
    ...
    void OnTriggerStay(Collider other)
    {
        if(other.gameObject.tag == "SpawnWall")
        {
            if (rend.enabled)
            {
                bewindWall = true;
                navAgent.Stop();
                //other.collider.SendMessageUpwards("RemoveBoard", SendMessageOptions.RequireReceiver);
            }
            else
            {
                navAgent.Resume();
